...
| **E18** universal park (ADR-0020/0037) — **default build since cycle 1**; `sleep.rs` deleted, its gate passes through park | `curl_exec` 100 × 200 ms **279 ms**, `pdo_pgsql` **296–333 ms** with no PHP hook and no offload (controls 20.3 / 20.6 s); through the project bench after the E18-I1 fix **301–308 ms**; gate ≈ 8 ns/call; E1/E2/E4/E5 on vs off indistinguishable from noise (E4 quiet band pending); phpt gate ≥ baseline; policy table `IGNIS_PARK` = `lib[:symbol]` rows, seed `libphp:sleep,libphp:usleep,libphp:nanosleep,libcurl,libpq,libssl,libcrypto` | V-45, V-46 |
