...
**Found by the 2026-09-18 audit and open** (`BACKLOG.md` has each with its acceptance): `R-HELP` (the binary has no `--help`), `S0-DOCS-UNVERIFIED`, `S0-RESPOND-START` — now half-closed, the dead registration went with V-91 — `S0-FRANK` (a frankenphp test regressed and is still unnamed), `R-DNS`, `R-PDO-SQLITE`, `R-FOREIGN-FIBER`, `R-SESS`, `R-REVIEW-CHORES`, `S-DBAL-DIRECT`, `S-POOL-LEASE-AGE`, `S-EXCLUSIVE`, `S-REQUEST-FORMATS`, `S-SERVE-SECOND-ADDRESS`, `S4-MIXED`, and `R-REVOLT-FLAKE` (the compat gate reads 79 or 80 on identical code). Plus eleven the audit read and did not fix, filed under "Found by the 2026-09-18 audit, not fixed in it" — the largest being `A-OUTPUT-FIBERKEY` (per-fiber output state keyed by address with no destroy hook, the V-72/V-76 bug class), `A-DESTRUCTOR-IO` (a destructor that parks) and `A-BACKEND-B-CI` (nothing anywhere builds backend (b)). `BACKLOG.md` holds 59 open items; the closed ones are indexed there and kept in full in `BACKLOG-CLOSED.md`.
