...
| **E15** compat suites — **PARTIAL** (E15a/b/c/d done, E15e open) | phpt main mode **108/108 fibers, 80/80 sockets, 131/138 streams** of stock, 0 upstream failures; Revolt `DriverTest` **81 tests / 222 assertions / 0 failures** = StreamSelectDriver; Swoole shim **44/153** with the missing-hook ranking; FrankenPHP testdata **29 pass / 4 fail / 33 skip** | V-22, V-23 + addendum |
...
- **E1 under CPU contention**: 1302 ms with two builds on the other cores (V-2 addendum); the 2–3% cold margin does not survive a loaded box. The warm pool (V-4) is the fix. Cycle 19's re-runs happened under the E15e porter's load and are **not recorded** — no quiet-box E1 number after the E13'/GC changes.
...
- **E2' (< 5 µs per warm job) with the superglobals observer on**: **not met** — 8.0–11.4 µs warm, cold per-fiber varying 3× because PHP's cycle collector walks every pooled fiber (V-11 addendum). E13' (lazy swap) and loop-scheduled GC landed afterwards; their effect is **not measured yet** on a quiet box (no V-n).
...
- **E15e chaos mode**: in progress (porter running at 03:56Z); `bench/e15-chaos.sh` is not in the repo yet and there is no V-n. H22e stays OPEN.
...
E15e (Symfony/Doctrine chaos mode — the only unfinished part of E15); quiet-box re-measurement of E2'/E13' (lazy swap + loop-scheduled GC) and of E1 after tonight's last changes; E12' fail-fast validation (code pushed, unmeasured) and mapping a fatal's in-flight request to 500 instead of a closed connection; E3' (10M requests, 4 threads, streams + state enabled); fiber-budget pool cap with request queueing (V-5 memory note); per-endpoint budget + circuit breaker (pain map PHP-FPM 2); in-process Table (RoadRunner 4); MySQL/Redis drivers; allocator-level leak detector; E9' (signals/queries/cancellation), E10' (client-streaming/bidi, TLS on the listener), E8' (multi-value `Set-Cookie`), E7' (AMPHP on hooked transports, signals); `PHP_BINARY`, `stream_socket_get_name`/`stream_select` on hooked streams, `stream_type` reported as `ignis_tcp` (V-23 fiber-mode failures).

## Ranked recommendation for the next 3 cycles

1. **Close the two missing numbers.** Finish E15e chaos mode (porter) and then re-measure E1/E2' on a quiet box with the E13' lazy swap and loop-scheduled GC in place — E2' is the only brief-level target whose current answer is "not met with the observer on" (V-11 addendum), and every per-fiber figure recorded after 03:35Z was taken under load.
