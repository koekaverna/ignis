...
| **M5-4** | Nightly perf job `agent` `done (validated by main: schedule+dispatch only, E1 1144.3 ms / E2 201.08 ms + 3.48 us on my run, dir=gt for throughput; the runner-side numbers wait for the first dispatch; **runner numbers landed 2026-09-22, V-119** — the schedule had died on `sh` for six nights; E1 1155–1257 ms on ubuntu-latest against a 1200 threshold, so the thresholds and the hello baseline need a week of scheduled runs before they are re-set)` |
