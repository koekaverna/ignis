...
### M3-5a Laravel in classic mode, serialised by the budget `agent` `CLOSED 2026-09-22 — out of the MVP (owner interview, DECISIONS)`
Laravel is out of the MVP scope; classic mode itself stays (the FrankenPHP compat gate runs through it).
...
### M3-5b Fiber-scoped `Container::$instance` and Facade caches `main` `CLOSED 2026-09-22 — out of the MVP (owner interview, DECISIONS)` — needed an ADR
Laravel is out of the MVP scope.
...
### R-PDO-SQLITE `new PDO('sqlite:…')` blocks the thread and cannot be routed per driver `main` `CLOSED 2026-09-22 — there is nothing to route to any more (the offload pool is deleted, DECISIONS)`
The documented answer is the whole answer now: `pdo_sqlite` blocks its thread for a local file read, as every regular file does (ADR-0024). The proxy design below was never built and is kept as text only.. **The premise moved.** This entry says routing `PDO` sends `pgsql` to a worker too — true when it was written, false since the same day: `route.rs` dropped `PDO` from `DEFAULT_CLASSES`, which is now `"SQLite3"` alone (V-59 addendum). The simplification the designed fix was meant to deliver **already shipped as a config default**, so the ~60-line proxy below should **not be built** — that is 60 lines never written, and it is the cheapest result in this whole reconciliation. What is left is the documented consequence: a `pdo_sqlite` user blocks the thread for a local file read, exactly as `file_get_contents` and opcache do (ADR-0024). The runtime picks the mechanism in `create_object`, which the VM calls when it executes `new` — before the constructor's arguments exist, so `pgsql:` and `sqlite:` are indistinguishable at the only moment it can choose. Routing the `PDO` class therefore sends `pgsql` to a worker too (2,753 ms against 303 ms parked for 100 × 200 ms queries, V-59 addendum), and not routing it leaves `pdo_sqlite` blocking the OS thread for the file access. Today the choice is `IGNIS_OFFLOAD_CLASSES`, per class and not per driver, and an application using both under concurrency cannot have both right. **The designed fix, ~60 lines, unbuilt:** give the proxy class the *original* `create_object` instead of plain object creation, so the proxy carries a real PDO payload; then decide inside `Proxy\PDO::__construct`, where the DSN is visible — `sqlite:` keeps today's worker handle, everything else calls `parent::__construct()` on the calling thread and every forwarder goes to `parent::` instead of the worker, i.e. it parks. `instanceof PDO`, type hints and constants are unaffected (the proxy already extends PDO); the only visible difference stays `$pdo::class`. Needs a suppression flag in `route.rs` so the nested `new` inside the constructor is not re-proxied (the `PASS` thread-local already exists for functions). **Gate before it lands:** `pdo_pgsql` still 303 ms (the extra forwarding layer must not cost), `pdo_sqlite` reaches the pool, and `PDOStatement`/`fetch()` behave as V-24 measured.

### S-CANCEL-IN-C Cancelling a fiber parked inside a C library holds the thread and answers nobody `main` `open — measured 2026-09-21 (V-117)`
**What.** A 500 ms deadline on a handler running `SELECT pg_sleep(5)` through `pdo_pgsql`: the thread
is held ~4.75 s more, the client is **never answered** (`http 000`), and `Loop::answer()` reads its
own parameter as undefined three times. The identical handler parked on `Ignis\sleep(5000)` answers
504 in 503 ms with the ticker beside it never missing a beat. E11's 0.28–0.78 ms cancellation number
covers only that second path; nothing covered this one.
**Why.** `throwInto` finds no entry in the PHP-side maps, because a C park lives in Rust's `wait.rs`
table, so `cancelCPark` resumes the fiber with the exception **on the loop's own stack**; `await_op`
sees `EG(exception)` and returns `None`; `park_pollfds` returns `None`; `park_io` maps that to
`Wait::Ready`; the handler falls through to the real blocking syscall. The undefined parameter is the
VM frame damaged by throwing into a fiber suspended inside a C frame — which `cancelCPark`'s own doc
block already half-states.
**Why it matters more than it looks.** A deadline exists to free the thread when a dependency is
slow, and this *is* that case; a client disconnect takes the same path (ADR-0009). The mechanism
fails at the only moment it is for, and the only trace is a watchdog line and three warnings.
**Two candidate designs, and the choice changes ADR-0009's contract, so it wants an ADR.**
(a) **Let the syscall fail instead of completing.** On a cancelled park, return an error to the C
...
**Acceptance.** The libpq arm above, with: the thread free within the same bound E11 uses, a 504
...
### R-2 MySQL and Redis native drivers (B4) `main` — park covers them today: mysqlnd and phpredis do their I/O through PHP's stream layer (V-114, V-116).
...
**Related, and already done** (2026-09-21, V-111): the eligibility test was refusing every
anonymous-inode descriptor — `eventfd`, `timerfd`, `signalfd`, `inotify` — which epoll accepts and the
reactor can wait on, so a direct blocking `read()` on one now parks. It does **not** cover a library's
own event loop, as V-111 first claimed and then corrected the same day: that means `epoll_wait`, which
is not interposed and which **no library in the default policy imports** (measured — `libcurl`,
`libpq`, `libssl`, `libphp`, zero each). Interposing it is therefore worth nothing on its own and
belongs to this entry rather than beside it.
...
### A-SWOOLE-TICKERS The Swoole shim polls instead of waiting `agent` `CLOSED 2026-09-22 — the shim is deleted (DECISIONS)`
Nothing left to fix.
...
### A-ADAPTER-MECHANISMS Three adapters carry a mechanism of their own `research` `open — 2026-09-18; one of the three (the offload proxy generator) went with its package on 2026-09-22`
...
### R-TA-REQUEST-SCOPE Upstream already specifies the embedder's role we occupy `research` `severity: planned` `DEFERRED 2026-09-22 — backend (b) is deleted (DECISIONS); reopen with it`
Research 48 stays as the record; there is no fork checkout in CI any more to read the headers against.
...
### R-TA-SERVER An async PHP server already exists next to the extension `research` `severity: n/a — scope question, not a defect` `priority: high` `DEFERRED 2026-09-22 — backend (b) is deleted (DECISIONS); reopen with it`
The scope question is answered for now by the MVP decision: one engine, mainline PHP 8.5.
...
| **R-PDO-SQLITE** | `pdo_sqlite` blocks the thread `main` `CLOSED 2026-09-22 — nothing to route to; documented as blocking (ADR-0024)` |
| **S-DBAL-DIRECT** | A service injecting `Doctrine\DBAL\Connection` shares it `main` `CLOSED 2026-09-22 — ignis/doctrine deleted; the shape is S-EXCLUSIVE` |
| **S-POOL-LEASE-AGE** | A hung fiber keeps its connection `main` `CLOSED 2026-09-22 — the pool is deleted; the runtime half is M4-7` |
| **A-SWOOLE-TICKERS** | The Swoole shim polls `agent` `CLOSED 2026-09-22 — the shim is deleted` |
| **R-TA-REQUEST-SCOPE** | Upstream's embedder role `research` `DEFERRED 2026-09-22 — backend (b) deleted` |
| **R-TA-SERVER** | Upstream's async server `research` `DEFERRED 2026-09-22 — backend (b) deleted` |
...
| **M5-4** | Nightly perf job `agent` `DELETED 2026-09-22 (owner)` — built, dead on `sh` for six nights, run three times once fixed: E1 1155–1550 ms and hello 58–81k rps for one binary on the shared runner (V-119). The owner's verdict: no use on rented hardware. `nightly.yml` and `bench/results/nightly-baseline.txt` removed, issue #1 closed; DECISIONS 2026-09-22 |
