...
### R-GLOBALS A legacy entry script loses its globals and cannot redeclare its functions `main` `open` — V-53, measured against stock `php -S`. In `Ignis\Classic` the entry is `include`d inside a fiber, in one long-lived PHP request, so (a) its top-level variables are locals of the closure: `$GLOBALS['wpdb']` is NULL and `global $wpdb` in any function sees NULL, every request; (b) a top-level `function foo()` survives into the next request: `Cannot redeclare`. Stock `php -S` and php-fpm are fine — a fresh request, a fresh scope. This blocks WordPress/Drupal-class apps, which is M3's "real apps unchanged". Symfony is unaffected (runtime returns a closure, state lives in objects — V-53). Options, none built: (1) run a classic entry on the thread's main context, not in a fiber — top-level assignments become real globals, and the function table still needs a reset; (2) a per-request reset of the function table and global scope (`zend_deactivate`-shaped), which gives back exactly the bootstrap cost the worker model exists to avoid; (3) declare it a non-goal and document that classic mode serves worker-shaped scripts only — cheapest, and the honest reading of what `Ignis\Classic` is today. The reproducer is in V-53; the same mechanism explains the five `Zend/tests/fibers/destructors_*` rows research 33 called a GC defect.

### R-SESS A session-file lock stalls the whole PHP thread, not one fiber `main` `open` — research 30 group (d): `ext/session/mod_files.c:210` takes `flock(LOCK_EX)` on the session file, and a regular file can never park (epoll refuses it), so `would_block` forwards and the OS thread waits for as long as another request holds that session. With one PHP thread per core and the classic "two tabs, same session" pattern this is a real stall, and it is the sharpest of the forwarded-anyway sites the audit found. Options, none built: route the session handler through offload (a worker thread blocks instead of the PHP thread), a runtime-owned session store (like ADR-0015's pg pool), or document it as a non-goal and tell users to pick a non-file handler. Needs a number first: a bench with N fibers on one thread sharing a session id.
