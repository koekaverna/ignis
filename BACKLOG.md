...
### S-DBAL-POOL A connection per request is parity with php-fpm, not the answer `main` `open — the price V-85 accepted`. Since V-85 every request that touches the database opens its own connection: about 1 ms of TCP plus SCRAM (V-45), and open connections equal to requests in flight rather than to threads. php-fpm without `pconnect` does exactly this, so it is not a regression against the baseline — but this project exists to be better than the baseline, and a pool that leases per statement is the way (`Ignis\Pg` already has the shape: lease per fiber, bounded, session reset on return, V-21). The missing piece is a DBAL driver or middleware over it, which ADR-0015's Consequences names as a follow-up and nobody has built. **Decide with a number first:** `pdo_pgsql` per query against `Ignis\Pg`'s 112 µs on the same box — V-21 recorded "Not measured yet" and it still is. If the overhead plus the handshake is within ~2× of the pool, this item stays parked and `Ignis\Pg` stays a demonstrator; if it is 5×, the driver earns its cycle.
