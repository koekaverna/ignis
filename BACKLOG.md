...
## E19 — Boot-heap snapshot, classic mode (owner, 2026-09-17; ADR first)

Owner's note, verbatim intent: boot allocations in a dedicated per-thread Zend MM arena; `mprotect` after boot; a `SIGSEGV` write barrier copies dirty pages to an undo log; request end restores dirty pages and discards the request arena without destructors. **Acceptance:** a Laravel app that leaks a static-held object per request runs 1M requests with RSS flat and identical response bytes; cost per request < 100 µs at < 50 dirty pages. **Kill criterion:** any extension whose C-state lives outside the arena and diverges after restore.

### E19-R1 What the barrier costs `main` `done (research 34)` — measured before designing anything, because the acceptance names a number. `mprotect`+`SIGSEGV` costs **7.1 µs per fault**, i.e. **357 µs per request at the specified 50 dirty pages — 3.5× over the budget**; a whole-arena memcpy with no barrier at all is **34 µs** at a 2 MiB arena but 678 µs at 16 MiB; soft-dirty bits are 70 µs but `clear_refs` is process-wide and breaks with more than one PHP thread. At a framework-sized boot heap none of the three meets 100 µs.

### E19-R2 How big is a real boot heap, and how many of its pages does a request write `main` `done (research 34 addendum): Symfony, not Laravel — the owner pointed out ../symfony-ignis was already here. Boot heap 2.0 -> 6.0 MiB; **11-12 dirty pages per request** (20 on the first), measured with soft-dirty bits and no barrier. At that count the proposed mprotect+SIGSEGV mechanism costs **71 us against its 100 us budget** and is the only one of the three that fits — the other two are O(arena) and a 6 MiB heap already puts them over.` This is the number the whole design rests on and it is currently an assumption. Arm A of `bench/e19/compare.c` doubles as the instrument: its fault count is exactly the dirty-page count.

### E19-R3 `UFFD_WP` as the barrier instead of `SIGSEGV` `main` `open` — a handler thread instead of signal delivery; still a context switch per fault (~2–4 µs), so it is only worth building if E19-R2 says the dirty set is small.

### E19-A ADR `main` `unblocked (E19-R2 answered)`. The mechanism to specify is the one the owner specified: at the measured 11-12 dirty pages it costs 71 us against the 100 us budget, and it is the only one of the three that does not grow with the boot heap. What the ADR must now settle is correctness, not cost: extension C-state outside the arena (the owner's own kill criterion), file descriptors opened during a request and not closed by discarding the arena, the skipped destructors as a visible semantic change against php-fpm, and the hard constraint that this needs one request at a time per thread (classic mode, budget.fibers = 1 — a parked fiber would share the request arena with the next request).
