...
### E19-R2 How big is a real boot heap, and how many of its pages does a request write `main` `blocked on M3-5a (Laravel is not installed)`. This is the number the whole design rests on and it is currently an assumption. Arm A of `bench/e19/compare.c` doubles as the instrument: its fault count is exactly the dirty-page count.
...
### E19-A ADR `main` `open — blocked on E19-R2`. Cannot be written honestly until the cost question has an answer: three of the possible mechanisms are already measured and the specified one is the most expensive.
