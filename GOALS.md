...
| E2 | `Ignis\all()` of three 200 ms calls returns in < 230 ms; per-fiber overhead < 100 µs | **CONFIRMED** 202 ms, 22 µs (V-3). Raised: E2' = per-fiber overhead < 5 µs with a warm fiber pool → met with the superglobals observer off (4.5 µs), **not met with it on** (8.0–11.4 µs, V-11 addendum): E13' lazy swap + loop-scheduled GC re-measured on a quiet box (V-28): observer cost now ≤ 0.7 µs (inside noise), warm 5.5–6.4 µs on / 4.9–5.7 µs off — E2' still not met, the observer is no longer the reason** on a quiet box (no V-n) |
