...
|---|---|---|
| `pm.max_children` in `www.conf` | `workers × threads × budget.fibers` — workers = master-forked processes filling the cores on the non-thread-safe engine (`1` on the thread-safe one), threads = OS worker threads per process filling the cores on the thread-safe engine (`1` on the non-thread-safe one), `budget.fibers` = concurrent request fibers admitted per thread; a request past that waits as data, not a blocked worker | ADR-0019, ADR-0044; sizing in [operate.md](operate.md) |
