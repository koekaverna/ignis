...
| [0041](0041-blocking-quality-gate.md) | A blocking quality gate, and what it costs | proposed | clippy/rustfmt/deny and phpstan/php-cs-fixer/phpunit block a merge from the first commit, so the debt is cleaned in the same body of work rather than deferred; the expensive half is deliberate (`undocumented_unsafe_blocks`, 92 sites) because CLAUDE.md already claimed that property and it was not true, and coverage is reported rather than gated on the Rust side because more than half those lines need a live PHP engine |
