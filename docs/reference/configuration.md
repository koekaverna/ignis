...
| `offload` | `IGNIS_OFFLOAD` | `0` (off) | Synchronous offload worker threads for `curl_*`/`PDO`/`SQLite3` (ADR-0016/E16). Each costs a full PHP thread. | Unlike `threads`/`budget.*`/`listen`, there is **no unconditional `default_env` call** for `offload` in `config.rs` — if the file is silent and `IGNIS_OFFLOAD` is unset, the env var is simply never set, and `main.rs`'s own `unwrap_or(0)` supplies the 0. Net effect is the same default, reached by a different path. |
...
| `IGNIS_OFFLOAD_FUNCTIONS` | `crates/ignis/src/php/route.rs` | `curl_init,curl_setopt,curl_setopt_array,curl_exec,curl_getinfo,curl_error,curl_errno,curl_close,curl_reset,curl_pause,curl_multi_init,curl_multi_add_handle,curl_multi_exec,curl_multi_select,curl_multi_getcontent,curl_multi_remove_handle,curl_multi_close,curl_multi_info_read` | Comma-separated internal function names auto-routed to the offload pool (E16 auto-routing) when running inside a fiber and the extension is loaded; unset means "the default list", not "nothing". |
| `IGNIS_OFFLOAD_CLASSES` | `crates/ignis/src/php/route.rs` | `PDO,SQLite3` | Comma-separated class names whose `new` is auto-routed to the offload pool inside a fiber. |
