...
| `IGNIS_DOCTRINE_POOL` | `php/packages/doctrine/src/Pool/PoolingMiddleware.php` | `0` — a connection per fiber | Connections per **thread** in pool mode; the process opens `threads × this` at most. `0` keeps the default mode, where every request that touches the database opens its own connection and closes it at request end (V-85). Either way a connection is only ever leased to one fiber at a time: libpq is not reentrant per connection. |
| `IGNIS_DOCTRINE_POOL_WARM` | `php/packages/doctrine/src/Pool/PoolingMiddleware.php` | the pool limit | Connections opened while the thread boots (`IgnisDoctrineBundle::boot()`), so the first requests skip the handshake and an unreachable database fails the boot instead of the first query. `0` fills the pool lazily. |
| `IGNIS_DOCTRINE_POOL_WAIT_MS` | `php/packages/doctrine/src/Pool/PoolingMiddleware.php` | `5000` | How long a fiber waits for a free connection once every one in the pool is leased, before `Ignis\Doctrine\Pool\PoolTimeoutException`. The fiber parks for this wait; the thread keeps serving. |
