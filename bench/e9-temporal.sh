...
echo "== replay (history fetched by Rust over gRPC) — must pass"; WORKFLOW_ID=demo-1 timeout 30 "$BIN" php/temporal/replay.php 2>&1 | tail -3
echo "== replay with the timer removed (DEMO_MUTATE=1) — must fail"; DEMO_MUTATE=1 WORKFLOW_ID=demo-1 timeout 30 "$BIN" php/temporal/replay.php 2>&1 | grep -E "evicted|REPLAY_" | tail -3
