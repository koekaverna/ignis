...
# is taken; readiness is the expected body, never "something answered".
...
wait_ignis() { # PID
  ...
  if [ "${up:-0}" != 1 ] || ! kill -0 "$1" 2>/dev/null; then
...
./target/release/ignis --threads 1 examples/grpc_server.php > /tmp/e10-ignis.log 2>&1 & P=$!; wait_ignis $P
run_ghz "$ADDR" "Ignis, PHP handler, 1 PHP thread"; kill $P; wait $P 2>/dev/null
./target/release/ignis --threads 4 examples/grpc_server.php > /tmp/e10-ignis.log 2>&1 & P=$!; wait_ignis $P
run_ghz "$ADDR" "Ignis, PHP handler, 4 PHP threads"; kill $P; wait $P 2>/dev/null
