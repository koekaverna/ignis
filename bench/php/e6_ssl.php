<?php

// Control arm renamed 2026-09-18: this bench switched `IGNIS_NO_STREAM_HOOK`, and the TLS records are ext/openssl's, parked by the interposer (V-49),
// so the variable had stopped doing anything — both arms measured the same build and the
// control could not fail. `IGNIS_NO_UNIVERSAL_PARK` is the one hook-off control today.

// E6' / H25: ssl:// through the hook. Env: PORTS="8441 8442 8443", CAFILE (the servers' self-signed cert).
declare(strict_types=1);
require __DIR__ . '/../../php/packages/runtime/src/ignis.php';
$ports = array_map('intval', explode(' ', getenv('PORTS') ?: '8441 8442 8443'));
$cafile = getenv('CAFILE') ?: '/tmp/e6-ssl/cert.pem';
$park = getenv('IGNIS_PARK');
// Two shapes of control: the whole interposer off, or the policy table emptied. e6-ssl.sh uses the
// second; printing "on" through it is how this label came to describe a control arm as hooked.
$hook = (getenv('IGNIS_NO_UNIVERSAL_PARK') || ($park !== false && trim($park) === '')) ? 'off' : 'on';

Ignis\async(static function () use ($ports, $cafile, $hook): void {
    // 1. Three concurrent https fetches, each server sleeps 200 ms: ≈ 200 ms with the hook, ≈ 600 ms without.
    $ctx = stream_context_create(['ssl' => ['verify_peer' => false, 'verify_peer_name' => false]]);
    $t = hrtime(true);
    $rs = Ignis\all(array_map(static fn(int $p) => Ignis\async(static fn() => file_get_contents("https://127.0.0.1:$p/", false, $ctx)), $ports));
    printf("hook %s: 3 concurrent https fetches (200 ms each) in %.0f ms; bodies: %s\n", $hook, (hrtime(true) - $t) / 1e6, json_encode(array_map('trim', $rs)));

    // 2. Verification follows the context, and **which way each case must go is part of the claim**:
    // a private CA is not in the system store, so the default must refuse; the cafile makes it pass;
    // a wrong peer_name must refuse; verify_peer_name=false makes that pass again; allow_self_signed
    // does not help a CA-signed leaf whose CA is unknown, so it must refuse too.
    //
    // Each case used to print `ok` or `FAIL` with no expectation beside it, so three correct refusals
    // read as three failures and nobody could tell them from the real thing — which is why this bench
    // was never gated and why nobody noticed its certificates had expired two days earlier and turned
    // the two passing cases red as well.
    $cases = [
        'default (verify_peer on)' => ['refuse', []],
        'cafile' => ['accept', ['cafile' => $cafile, 'peer_name' => 'localhost']],
        'cafile + wrong peer_name' => ['refuse', ['cafile' => $cafile, 'peer_name' => 'example.invalid']],
        'cafile + verify_peer_name=false' => ['accept', ['cafile' => $cafile, 'peer_name' => 'example.invalid', 'verify_peer_name' => false]],
        'allow_self_signed' => ['refuse', ['allow_self_signed' => true, 'peer_name' => 'localhost']],
    ];
    $wrong = 0;
    foreach ($cases as $label => [$expected, $opts]) {
        $r = @file_get_contents("https://127.0.0.1:{$ports[0]}/", false, stream_context_create(['ssl' => $opts]));
        $actual = $r === false ? 'refuse' : 'accept';
        $err = error_get_last()['message'] ?? '';
        $wrong += $actual === $expected ? 0 : 1;
        printf(
            "verify [%s]: %s (expected %s)%s\n",
            $label,
            $actual === $expected ? $actual : strtoupper($actual) . ' — WRONG',
            $expected,
            $r === false ? ' — ' . preg_replace('/\s+/', ' ', substr($err, 0, 200)) : '',
        );
        error_clear_last();
    }
    printf("e6_ssl_verify: cases=%d wrong=%d\n", count($cases), $wrong);

    // 3. STARTTLS: tcp:// connect, then stream_socket_enable_crypto() upgrades the hooked stream in place.
    $c = stream_socket_client("tcp://127.0.0.1:{$ports[1]}", $e, $s, 5, STREAM_CLIENT_CONNECT, stream_context_create(['ssl' => ['verify_peer' => false, 'peer_name' => 'localhost']]));
    $meta = stream_get_meta_data($c)['stream_type'];
    $ok = stream_socket_enable_crypto($c, true, STREAM_CRYPTO_METHOD_TLS_CLIENT);
    fwrite($c, "GET / HTTP/1.0\r\nHost: localhost\r\n\r\n");
    $resp = stream_get_contents($c);
    printf("starttls on a %s stream: enable_crypto=%s, response=%s\n", $meta, var_export($ok, true), json_encode(trim(substr(rtrim($resp), (int) strrpos(rtrim($resp), "\n")))));

    if ($wrong > 0 || $ok !== true) {
        exit(1);
    }
})->await();
