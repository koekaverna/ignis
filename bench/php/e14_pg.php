...
$main = Ignis\async(static function () use ($pool, $poolSize, $fibers, $n): void {
  try {
    ...
    if (!$seen) { echo "reset: backend $pidA not re-leased in 50 tries\n"; }
    ...
    printf("types: %s\n", json_encode($r[0]));
    try { $pool->query('SELECT $1::numeric AS n', [1]); } catch (Ignis\Pg\QueryError $e) { printf("unsupported type is an error: %s\n", substr($e->getMessage(), 0, 90)); }
    ...
    $l = $pool->acquire();
    $t = hrtime(true);
    for ($i = 0; $i < $n; $i++) { $l->query('SELECT 1 AS x'); }
    $seq = (hrtime(true) - $t) / 1e3 / $n;
    $l->release();
    $t = hrtime(true);
    for ($i = 0; $i < $n; $i++) { $pool->query('SELECT 1 AS x'); }
    $pq = (hrtime(true) - $t) / 1e3 / $n;
    $t = hrtime(true);
    $fs = [];
    for ($i = 0; $i < $n; $i++) { $fs[] = Ignis\async(static fn () => $pool->query('SELECT 1 AS x')); }
    Ignis\all($fs);
