...
$main = Ignis\async(static function () use ($fibers, $ms): void {
    ...
    if (extension_loaded('pdo_pgsql')) {
        ...
        printf(
            "pdo_pgsql auto-routed: %d x %d ms `new PDO` + query in fibers, %d workers: %.0f ms wall (bound ceil(%d/%d) x %d = %d ms); distinct backend pids: %d\n",
            $fibers,
            $ms,
            $w,
            $wall,
            ...
            $w,
            $ms,
            (int) ceil($fibers / $w) * $ms,
            count(array_unique($pids)),
        );
    } else {
    ...
    if (function_exists('curl_init')) {
        ...
        curl_setopt($ch, CURLOPT_WRITEFUNCTION, static function ($h, string $data) use (&$chunks, &$bytes): int {
            $chunks++;
            $bytes += strlen($data);
            return strlen($data);
        });
