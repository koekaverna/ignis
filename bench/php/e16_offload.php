...
$main = Ignis\async(static function () use ($fibers, $ms, $n, $workers): void {
    ...
    $ticks = 0;
    $stop = false;
    $ticker = Ignis\async(static function () use (&$ticks, &$stop): void { while (!$stop) { Ignis\sleep(10); $ticks++; } });
    $t0 = hrtime(true);
    $fs = [];
    for ($i = 0; $i < $fibers; $i++) { $fs[] = Ignis\async(static fn () => Ignis\offload('e16_slow', $ms)); }
    $rs = Ignis\all($fs);
    $wall = (hrtime(true) - $t0) / 1e6;
    $stop = true;
    $threads = count(array_unique(array_column($rs, 'thread')));
    printf("blocking calls: %d x %d ms through %d workers: %.0f ms wall (bound = ceil(%d/%d) x %d = %d ms); distinct worker threads used: %d; fiber thread ticked %d times (10 ms sleeps) meanwhile\n",
        $fibers, $ms, $workers, $wall, $fibers, $workers, $ms, (int) ceil($fibers / $workers) * $ms, $threads, $ticks);
    ...
    foreach (['empty' => [], '1KB' => array_fill(0, 64, str_repeat('x', 12)), '64KB' => array_fill(0, 1024, str_repeat('y', 60))] as $label => $payload) {
        $t = hrtime(true);
        for ($i = 0; $i < $n; $i++) { Ignis\offload('e16_echo', $payload); }
    ...
    try { Ignis\offload('e16_throw'); echo "exception: NOT propagated\n"; }
    catch (Ignis\Offload\RemoteException $e) { printf("exception: %s(%s, code %d) propagated as RemoteException\n", $e->remoteClass, $e->getMessage(), $e->getCode()); }
    ...
    if (extension_loaded('pdo_pgsql')) {
        $t0 = hrtime(true);
        $fs = [];
        for ($i = 0; $i < $fibers; $i++) { $fs[] = Ignis\async(static fn () => Ignis\offload('e16_pg', $dsn, $ms / 1000)); }
        $rs = Ignis\all($fs);
        $wall = (hrtime(true) - $t0) / 1e6;
