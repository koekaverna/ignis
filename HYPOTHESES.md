...
|---|---|---|---|---|---|
| H0 | PHP 8.5.10 builds here as ZTS + embed with the minimal extension set and `php -r 'echo PHP_ZTS;'` prints 1. | `scripts/build-php.sh`, `/opt/php85-zts/bin/php -r 'var_dump(PHP_ZTS, extension_loaded("pdo_sqlite"));'` | ZTS=1, all listed extensions loaded | 30 min | see V-0 |
| H1 | A Rust binary linking libphp (ZTS, bindgen) can register an internal module and run a script that calls a Rust-implemented function; startup+script < 50 ms. | `cargo run -p ignis -- examples/hello.php` | prints "hello from rust: 42" | 90 min | see V-1 |
