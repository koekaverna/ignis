...
| **M2 Install** | get a binary or an image and be serving in under two minutes without building PHP | **Image DONE 2026-09-16 (V-39)** — `docker run ghcr.io/koekaverna/ignis` serves the hello entry, 64 MB, `ldd` clean, unprivileged; `image.yml` builds, pushes and smoke-tests the pushed image on every push to main. **Open: a static binary** — no `libphp.a` and ~35 shared libraries behind libcurl, so it is a PHP rebuild with a trimmed curl. | B6 (static artifact, `php-image.yml`) |
