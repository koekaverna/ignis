...
| **M2 Install** | get a binary or an image and be serving in under two minutes without building PHP | **DONE 2026-09-16 (V-39 + addendum)** — `docker run ghcr.io/koekaverna/ignis` serves the hello entry, 64 MB at the time and **84 MB today** (V-83, after the toolchain extensions and `libxml2`), `ldd` clean, unprivileged; `image.yml` builds, pushes and smoke-tests the pushed image on every push to main, and its first run on `574b231` was green. **Open: a static binary** — no `libphp.a` and ~35 shared libraries behind libcurl, so it is a PHP rebuild with a trimmed curl. | B6 (static artifact, `php-image.yml`) |
| **M3 Real apps unchanged** | run a Symfony skeleton through `ignis serve` with a documented recipe, and read a compat table that says what works unchanged and what does not | **Symfony DONE 2026-09-16 (V-40)** — `ignis/runtime` composer package (`php/composer.json`), `extra.runtime.class`, entry = `public/index.php`; bare skeleton served from the image, 20/20 concurrent. **Laravel: out of the MVP (owner, 2026-09-22, DECISIONS)** — it had been re-scoped (BACKLOG M3-5a/M3-5b, both closed): M3-5a ships Laravel in classic mode with `budget.fibers = 1` per thread (ADR-0019 turns `php/packages/runtime/src/classic.php`'s "must not suspend" assumption into a guarantee, Octane's own model), M3-5b is the ADR for a fiber-scoped `Container::$instance`/Facade cache on the ADR-0006 observer, the route to real concurrency at `budget.fibers = 1024` — plus **Packagist publication** (M3-6) and the recipe in README. The compat table is in README. | E8', research 24 (libpq path) |
| **M4 Operate** | see what the server is doing and survive a bad dependency: `/_ignis/metrics`, graceful reload, a cap on connections | **partly done.** Metrics (V-55, 19 families today), the two-phase drain on `SIGTERM`/`SIGINT` (V-56) and reload without restart (V-90) are in. The bulkhead and the lease hold-time were built for the runtime-owned PostgreSQL pool and removed with it on 2026-09-18 (ADR-0015 closed, V-87) — a bounded wait now lives in `ignis/doctrine` (V-85) and a breaker does not exist. Still open: the connection cap, so RSS stays under a ceiling while 100k clients hold connections | B2, B8, Phase D metrics + reload, hold-time tracking |
| **M5 Ship** | download a versioned release with a changelog, and follow a migration guide from php-fpm / FrankenPHP / RoadRunner | **First release published 2026-09-17 (V-57): `v0.1.0-rc.1`, image + tarball, both verified by download.** The tag exposed a packaging bug no audit could see; the nightly perf job (M5-4) ran on 2026-09-22 and was deleted the same day (DECISIONS: shared runners, V-119). Previously: workflows written, not yet exercised — `.github/workflows/release.yml` (build + push the tagged image, `ignis --version` == tag, binary + `libphp.so` tarball, release notes) and `nightly.yml` (E1/E2'/hello/E14/E16/B1-p99 vs thresholds) both dry-run clean but have not run against a real `v*` tag or a real schedule dispatch — the tag push is refused by the session git proxy (owner action). Docs done: `docs/migrate.md` (M5-2) and `docs/operate.md` (M5-3), every claim with a V-n. | Phase D docs |
...
| M4 Operate | ADR-0022 (observability), ADR-0009 addendum (deadlines/cancellation gaps), ADR-0025 (memory: the connection cap), ADR-0030 (preemption, proposed). ADR-0015 was here for the pool rules and is closed — the code is deleted (V-87) |
| M5 Ship | ADR-0027 (distribution), ADR-0023 (validation rules), ADR-0035 (security), ADR-0036 (naming) |
| E18 Universal park | ADR-0020 (+ addendum: the owner's design element by element), ADR-0021 (layers), ADR-0016 addendum (offload boundary), **ADR-0037, accepted 2026-09-18 (consolidation to three mechanisms; research 29 inventory, research 30 libphp audit gate). Cycles 1–3 all landed: universal park is the default build, the `sleep()`/`ext/sockets`/`stream_socket_accept()` hooks are deleted (V-46, V-47, V-48) and so are the stream transport factory and the rustls path (V-49, §6 step 4). Three mechanisms left — park, offload, context** |
...
**Deferred from the product, kept as R&D**: B3 in-process Table, B4 native MySQL/Redis (park already
covers them, V-114/V-116), B5 allocator-level leak detector, B7 TLS read-ahead in `stream_select`
...
| B4 | MySQL and Redis as native drivers | Swoole 5, ADR-0016 | **Re-opened as a question, not a task.** The E14 lease shape this row copied was deleted on 2026-09-18 (V-86 measured the runtime-owned PostgreSQL pool slower than parked `pdo_pgsql`; V-87 removed it), so a native driver has to earn its place against the same control: park `mysqli`/`phpredis` and beat it, or do not build it |
...
| B7 | ~~A6: TLS read-ahead invisible to `stream_select()`~~ **RETIRED 2026-09-18** | research 23 | The design was an eventfd handed out by `op_cast` in a rustls-backed stream. Both are gone (V-49): TLS is PHP's own `ext/openssl` over parked syscalls, PHP's native `has_buffered_data` applies, and V-49 records the case PASS in 22.9 µs. Nothing to build |
...
- **E11'** cancellation on client disconnect beyond the request fiber (extends V-14). The offload half went with the pool (2026-09-22) and the pgsql half with the runtime-owned pool (V-87): a parked `pdo_pgsql` query is cancelled by the same mechanism as any other parked syscall, or it is not cancellable at all — which is the question, and it has no measurement.

## Phase D — productization (parallel to B/C, scribe + bencher)

- Config file (threads, fiber budget) replacing the env flags; `ignis serve` / `ignis run` CLI.
- `/metrics` (Prometheus) from the reactor: in-flight, parked fibers, watchdog stalls, restarts.
- Graceful reload (drain + respawn per thread, opcache untouched — ADR-0012 already has the mechanism).
- ~~Nightly perf job in CI with thresholds (E1, E2', E4 hello, E14, E16) so regressions surface without a human reading numbers.~~ Built, run, deleted (owner, 2026-09-22): on shared runners the thresholds measure the neighbour (V-119). Regressions are caught by the VALIDATION discipline on the developer box, not by CI.
