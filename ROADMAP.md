...
| A2 | Real-timer path: one `tokio::spawn` per sleep/watch → a timer wheel (`DelayQueue`) inside the reactor task | V-28 addendum: 5.5–6.4 µs warm per parked fiber vs 4.5 with the zero-sleep fast path | `sleep(1)` round trip < 5 µs warm; E1 overhead < 150 ms |
