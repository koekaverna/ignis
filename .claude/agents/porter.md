...
- Every failure gets one of three labels with a one-line reason: **ours** (Ignis runtime/harness bug), **not applicable** (needs php-cli/CGI semantics, an extension we do not build, a Caddy/Go-only feature, an external host), **upstream** (fails identically on /opt/php85-zts/bin/php).
