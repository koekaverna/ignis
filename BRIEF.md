...
E18 Universal park (added by the owner, 2026-09-16, ADR first): export the ~25 blocking libc symbols from the ignis binary with a thread-local "fiber active" gate and caller-library policy; keep the stream factory as-is. Acceptance: (1) `curl_exec` inside a fiber parks with NO PHP-level hook and no offload — 100 × 200 ms on one thread ≈ 200 ms, CURLOPT_WRITEFUNCTION runs in the same fiber; (2) pdo_pgsql the same, offload disabled; (3) `getaddrinfo` parks via the runtime resolver; (4) non-fiber path overhead per syscall < 20 ns; (5) a deliberate mutex-holding library under policy "park" must deadlock in the test — proving the hazard is real — and the same test under "block" must pass. Kill criterion: any OpenSSL or libcurl test failing under "park" with a lock in the trace.
